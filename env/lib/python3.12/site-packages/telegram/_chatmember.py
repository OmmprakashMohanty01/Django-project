    ...
    * :class:`telegram.ChatMemberBanned`
    ...
    def de_json(cls, data: JSONDict, bot: Optional["Bot"] = None) -> "ChatMember":
        ...
        if "until_date" in data:
            ...
            data["until_date"] = from_timestamp(
                data.get("until_date"), tzinfo=loc_tzinfo
            )

        # This is a deprecated field that TG still returns for backwards compatibility
        # Let's filter it out to speed up the de-json process
        if (
            cls is ChatMemberRestricted
            and data.get("can_send_media_messages") is not None
        ):
            api_kwargs = {
                "can_send_media_messages": data.pop("can_send_media_messages")
            }
