...
class CaptionEntity(MessageFilter):
    ...
    def filter(self, message: Message) -> bool:
        return any(
            entity.type == self.entity_type for entity in message.caption_entities
        )
...
class _Dice(MessageFilter):
    ...
    def __init__(
        self, values: Optional[SCT[int]] = None, emoji: Optional[DiceEmojiEnum] = None
    ):
        super().__init__()
        ...
        if emoji:  # for filters.Dice.BASKETBALL
            ...
            if (
                ...
                self.name = (
                    f"filters.Dice.{emoji.name.title().replace('_', '')}({self.values})"
                )
    ...
    class FileExtension(MessageFilter):
        ...
        def __init__(self, file_extension: Optional[str], case_sensitive: bool = False):
            ...
            if file_extension is None:
                ...
                self.name = (
                    f"filters.Document.FileExtension({file_extension.lower()!r})"
                )
...
class Mention(MessageFilter):
    ...
    def __init__(self, mentions: SCT[Union[int, str, TGUser]]):
        ...
        if isinstance(mentions, Iterable) and not isinstance(mentions, str):
            self._mentions = {
                self._fix_mention_username(mention) for mention in mentions
            }
        else:
            self._mentions = {self._fix_mention_username(mentions)}

    @staticmethod
    def _fix_mention_username(
        mention: Union[int, str, TGUser]
    ) -> Union[int, str, TGUser]:
...
class StatusUpdate:
    ...
    CHAT_BACKGROUND_SET = _ChatBackgroundSet(
        name="filters.StatusUpdate.CHAT_BACKGROUND_SET"
    )
    ...
    GIVEAWAY_COMPLETED = _GiveawayCompleted(
        name="filters.StatusUpdate.GIVEAWAY_COMPLETED"
    )
...
class SuccessfulPayment(MessageFilter):
    ...
    def __init__(
        self, invoice_payloads: Optional[Union[list[str], tuple[str, ...]]] = None
    ):
