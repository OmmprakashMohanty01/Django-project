...
class Application(
    ...
    def __init__(
        ...
        *,
        ...
        self.__create_task_tasks: set[asyncio.Task] = (
            set()
    ...
    def __run(
        ...
        try:
            loop.run_until_complete(
                self._bootstrap_initialize(max_retries=bootstrap_retries)
            )
            if self.post_init:
                loop.run_until_complete(self.post_init(self))
            if self.__stop_running_marker.is_set():
                _LOGGER.info(
                    "Application received stop signal via `stop_running`. Shutting down."
                )
                return
            loop.run_until_complete(
                updater_coroutine
    ...
    async def _update_fetcher(self) -> None:
        try:
            ...
            while not self.update_queue.empty():
                _LOGGER.debug(
                    "Dropping pending update: %s", self.update_queue.get_nowait()
                )
    ...
    async def __process_update_wrapper(self, update: object) -> None:
        try:
            await self._update_processor.process_update(
